...
void addUser(Profile user, String imagePath) async {
  ...
  await db.collection("users").doc(map["uid"]).set(map).whenComplete(
      () => print('DocumentSnapshot added with ID: ${map["uid"]}'));
}
...
Future<void> fetchUser() async {
  Map<String, dynamic>? data =
      await localDb.collection("data").doc("user").get();
  if (data == null) {
...
void pushFood(FoodItem foodItem) async {
  ...
  await db.collection("foods").add(foodItem.toFirestore()).then(
      (documentSnapshot) => print(
          "Added Food with docID: ${documentSnapshot.id} and Name ${foodItem.name} and UniqueID ${foodItem.id}"));
}
...
void testFood(FoodItem foodItem) async {
  ...
  while (!isUnique) {
    ...
    await db
        ...
        .get()
        .then((querySnapshot) {
      isUnique = (querySnapshot.size == 0);
      print("notUnique");
    });
  }
  FoodItem temp = FoodItem(
      name: foodItem.name,
...
Future<List<num?>> fetchAvgRating(int fid) async {
  double? avg;
  int? reviews;
  var query = db.collection('ratings').where("fid", isEqualTo: fid);
  await query.count().get().then((value) {
    reviews = value.count;
  });
  await query.aggregate(average("rating")).get().then((aggregateQuerySnapshot) {
...
void testFetch() {
  int fid = availFood.keys.firstWhere((element) =>
      availFood[element]!.name.toLowerCase().trim() ==
      "Hot Pizza".toLowerCase().trim());
  fetchAvgRating(fid);
}
...
void addRating(int fid, double rating, String review) async {
  ...
  if (isExists) {
    print(
        "update $fid ${FirebaseAuth.instance.currentUser!.uid} ${availFood[fid]?.name}");
    try {
    ...
    print(
        "create $fid ${FirebaseAuth.instance.currentUser!.uid} ${availFood[fid]?.name}");
    try {
